            ...
            <>
              ...
              {editingId ? (
                <Card className="mt-5 p-5 sm:p-7">
                  <form className="space-y-6" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); void saveItem(); }}>
                  <div><h2 className="text-lg font-semibold text-white">{editingId === "new" ? "Add menu item" : "Edit menu item"}</h2>
                  <p className="mt-1 text-sm text-muted">Keep item details concise and customer-friendly.</p></div>
                  <div className="grid gap-5 sm:grid-cols-2">
                    ...
                    <label className="block space-y-2 sm:col-span-2" htmlFor="menu-description">
                      ...
                      <textarea aria-describedby={errors.description ? "menu-description-error" : undefined} aria-invalid={Boolean(errors.description)} className={`min-h-28 w-full resize-y rounded-xl border bg-white/5 px-4 py-3 text-sm text-foreground outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 ${errors.description ? "border-red-400" : "border-white/15"}`} id="menu-description" maxLength={2000} onChange={(event) => updateValue("description", event.target.value)} value={values.description ?? ""} />
                      {errors.description ? <span className="block text-sm text-red-300" id="menu-description-error">{errors.description}</span> : null}
                    </label>
                  ...
                  <div className="flex flex-col-reverse gap-3 border-t border-white/10 pt-5 sm:flex-row sm:justify-end"><SecondaryButton disabled={saving} onClick={cancelEdit}>Cancel</SecondaryButton><PrimaryButton disabled={saving} type="submit">{saving ? "Saving…" : "Save item"}</PrimaryButton></div>
                  </form>
                </Card>
              ) : null}
